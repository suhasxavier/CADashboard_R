...
function(input, output,session)
{
  observe({
    ...
    {
      print(e_id)
      taiga=function(eid_val,admin_flag)
      {
      ...
      # notifications only for half the teams
        ...
        #Skip tracking activity when admin logs in
        if(admin_flag==0)
        {
        noti_clicked=""
        ...
        observe({
          ...
          if(length(pwd_entered)>0)
          {
            if(pwd_entered=="a5uP0l4!")
            {
              ...
              t_names=as.character(unique(admin_file$name))
              output$admin_view=renderUI(selectInput("namelist", "Select a Team",t_names,selected = NULL))
              observe({
                if(!is.null(input$namelist))
                {
                  t_selected=as.character(input$namelist)
                  selected_members=as.character(admin_file[admin_file$name==t_selected,"full_name",])
                  output$drop_down_students=renderUI(selectInput("dropdownmembers","Select a student to view dashboard",selected_members,selected = NULL, multiple = TRUE))
                }
                user_selected=input$dropdownmembers
                
                print(user_selected)
                user_selected_email=as.character(tolower(admin_file[admin_file$full_name==user_selected,"email"]))
                print(user_selected_email)
                taiga(user_selected_email,1)
              })
            }
          }
        })
