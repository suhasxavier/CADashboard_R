  ...
  observe({
    ...
    if(substr(e_id,nchar(e_id)-3,nchar(e_id))==".edu" || substr(e_id,nchar(e_id)-3,nchar(e_id))==".com")
    {
      ...
      if(nrow(u_data)>0)
      {
        ...
        u_all_taiga_data=tail(u_all_taiga_data,7)
        u_taiga_data=u_all_taiga_data[c(1,2,3,7)]
        ...
        output$tabs1=renderUI(
          tabsetPanel(type="tab",
                      tabPanel("Overall Activity",h3("Overall Activity Monitor"),br(),showOutput("Spiderweb","highcharts")),
                      tabPanel("Taiga",h3("Your Taiga activity monitor"),showOutput("stackchart1","highcharts"),br(),h4("Taiga activity comparator"),showOutput("stackchart4","highcharts")),
...
#           plot <- Highcharts$new()
#           table = data.frame(id = c("Github Impact","Taiga Impact","GitHub Frequency","Taiga Frequency"),
#                              value = c(5,4.6,5,4.8),
#                              value1 = c(3.6,3.8,4,4.2),
#                              value2=c(3,2.8,2.5,3))
